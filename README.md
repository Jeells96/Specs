...
4. Add vendor submittals: drag PDFs, a **folder**, or a `.zip` onto the **Add Vendor Submittals** zone (or use "Choose a folder…"). Each PDF is matched to its section by the CSI number in its **folder name or filename** and turned into a package — so a folder like `23 07 19 - HVAC Piping Insulation/…/Submittal.pdf` lands on the right section even when the filename itself is generic. Anything that can't be matched automatically drops into a **Needs matching** panel where you pick its section from a dropdown (or skip it) — nothing is silently discarded. Everything uploaded shows in the **Uploaded submittals** list on the workspace, and each section card shows a 📎 count. You can also open a single section and create/upload a package by hand. Set statuses, dates, and overrides as the review progresses, and export the register when the GC asks.
